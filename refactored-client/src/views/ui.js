...
import { delta, canw, canh, canw2, canh2, ctxDrawImage } from "../canvas";
import { sprite, client, ui, fake_world, user, mouse } from "../game";
import { IMAGES } from "../graphics/images";
import { SPRITE, RARITY, COSMETICS, WORLD } from "../constants";
import { BUTTON_CLICK, gui_render_text, gui_create_button, gui_create_image_hd, get_mouse_pos, MOUSE_MOVE, MOUSE_DOWN, MOUSE_UP } from "../graphics/gui";
import { draw_fake_world } from "../graphics/flat";
import { lerp, ease_in_out_quad } from "../utils";
...
export default function UI(can, ctx) {
  ...
  this.update = function () {
    ...
    for (let i = 0; i < this.bkgd.length; i++)
  ...
  this.draw = function () {
    draw_fake_world();
    ui.xp = lerp(ui.xp, ui.xp_dest, 0.03);
    ...
    for (let i = 2; i < this.bkgd.length; i++) {
      if (this.bkgd[i].active === __DISPLAY__)
        this.bkgd[i].draw(ctx);

    }

    const remains = this.kit - Date.now();
    if (remains > 0) {
      let min = Math.floor(remains / 60000);
      ...
      const remain_box = this.bkgd[BAG_REMAIN_BOX];
      remain_box.draw(ctx);

      let img = this.kit_img_hour;
    ...
    if ((this.current_view & ((((__COSMETICS__ | __BAG__) | __BOOK__) | __SKIN__) | __ACCESSORY__)) > 1) {
      ...
      let img = sprite[SPRITE.BAG][this.bag][this.day_mode];
    ...
    if ((this.current_view & (((((__BAG__ | __BOOK__) | __SKIN__) | __ACCESSORY__) | __CRATE__) | __LOOT__)) > 1) {
      for (let i = 1; i < this.buttons.length; i++) {
        ...
        if (button.cosmeticId >= 0) {
          let img = button.cosmeticType[button.cosmeticId].day;
    ...
    if (client.privateServer === 0) {
      ...
      if (hint > 0) {
        let img = IMAGES.NORMAL_MODE_HOVER;
        ctxDrawImage(ctx, img, canw2 - (img.width / 4), -36 - ((img.height * ease_in_out_quad(1 - hint)) / 2), img.width / 2, img.height / 2);
      }
      var hint = this.buttons[FOREST_MODE].hint;
      if (hint > 0) {
        let img = IMAGES.FOREST_MODE_HOVER;
        ctxDrawImage(ctx, img, canw2 - (img.width / 4), -36 - ((img.height * ease_in_out_quad(1 - hint)) / 2), img.width / 2, img.height / 2);
      }
      var hint = this.buttons[MODE_COMMUNITY].hint;
      if (hint > 0) {
        let img = IMAGES.MODE_COMMUNITY_HOVER;
        ctxDrawImage(ctx, img, canw2 - (img.width / 4), -36 - ((img.height * ease_in_out_quad(1 - hint)) / 2), img.width / 2, img.height / 2);
      }
      var hint = this.buttons[MODE_EXPERIMENTAL].hint;
      if (hint > 0) {
        let img = IMAGES.MODE_EXPERIMENTAL_HOVER;
        ctxDrawImage(ctx, img, canw2 - (img.width / 4), -36 - ((img.height * ease_in_out_quad(1 - hint)) / 2), img.width / 2, img.height / 2);
      }
      var hint = this.buttons[VAMPIRE_MODE].hint;
      if (hint > 0) {
        let img = IMAGES.VAMPIRE_MODE_HOVER;
        ctxDrawImage(ctx, img, canw2 - (img.width / 4), -36 - ((img.height * ease_in_out_quad(1 - hint)) / 2), img.width / 2, img.height / 2);
      }
      var hint = this.buttons[ZOMBIE_MODE].hint;
      if (hint > 0) {
        let img = IMAGES.ZOMBIE_MODE_HOVER;
        ctxDrawImage(ctx, img, canw2 - (img.width / 4), -36 - ((img.height * ease_in_out_quad(1 - hint)) / 2), img.width / 2, img.height / 2);
      }
    }
  ...
  function update_subview(view, mode, _css) {
    for (let i = 0; i < _this.bkgd.length; i++) {
    ...
    for (let i = 0; i < _this.css.length; i++) {
  ...
  function init_profile(id, season) {
    ...
    if (id >= 0) {
      ...
      var kill = 0;
      for (let i = 0; i < ui.kill.length; i++)
        kill += ui.kill[i];
      var time = 0;
      for (let i = 0; i < ui.time.length; i++)
        time += ui.time[i];
  ...
  this.buttons[COUNTER++].view = __SKIN__;
  var FIRST_BUTTON_ACCESSORY = COUNTER;
  this.buttons[COUNTER] = gui_create_button(178, 182, "", [IMAGES.WOOD_ITEM_OUT, IMAGES.WOOD_ITEM_IN, IMAGES.WOOD_ITEM_CLICK], __HD__, __NO_BREATH__, undefined, -29, 205, __MIDDLE_X__ | __TOP__, __HIDE__);
  ...
  this.buttons[COUNTER++].view = __BOOK__;
  var FIRST_BUTTON_CRATE = COUNTER;
  this.buttons[COUNTER] = gui_create_button(178, 182, "", [IMAGES.WOOD_ITEM_OUT, IMAGES.WOOD_ITEM_IN, IMAGES.WOOD_ITEM_CLICK], __HD__, __NO_BREATH__, undefined, -29, 205, __MIDDLE_X__ | __TOP__, __HIDE__);
